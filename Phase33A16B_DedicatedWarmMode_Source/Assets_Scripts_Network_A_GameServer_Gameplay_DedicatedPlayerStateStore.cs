    ...
    public class DedicatedPlayerStateStore : MonoBehaviour
    {
            ...
            long serverTimeUnixMs)
        {
            ...
            lock (syncLock)
            {
                ...
                {
                    dict_stateByUserId.TryGetValue(SafeTrim(session.userId), out oldRecord);
                }
            ...
            lock (syncLock)
            {
                ...
                if (!string.IsNullOrWhiteSpace(safeUserId) &&
                    dict_stateByUserId.TryGetValue(safeUserId, out DedicatedPlayerStateRecord previousRecord) &&
                    previousRecord != null)
                {
                ...
                if (!string.IsNullOrWhiteSpace(safeUserId))
                {
                    dict_stateByUserId[safeUserId] = record;
                }
            }
        ...
        public bool RemoveByConnectionId(string connectionId, string reason)
        {
            ...
            lock (syncLock)
            {
                ...
                if (!string.IsNullOrWhiteSpace(safeUserId))
                {
                    ...
                    {
                        dict_stateByUserId[safeUserId] = removed;
                    }
                    else if (dict_stateByUserId.TryGetValue(safeUserId, out DedicatedPlayerStateRecord mappedRecord) && mappedRecord == removed)
                    {
                        dict_stateByUserId.Remove(safeUserId);
                    }
                }
            }
        ...
        public DedicatedPlayerStateRecord GetByUserId(string userId)
        {
            ...
            lock (syncLock)
            {
                return dict_stateByUserId.TryGetValue(safeUserId, out DedicatedPlayerStateRecord record) ? record : null;
            }
        }
        ...
        public bool RebindConnectionForUser(string userId, string newConnectionId, DedicatedPlayerSession session = null, string reason = "reconnect_rebind")
        {
            ...
            lock (syncLock)
            {
                if (!dict_stateByUserId.TryGetValue(safeUserId, out record) || record == null)
                {
                    return false;
                }
