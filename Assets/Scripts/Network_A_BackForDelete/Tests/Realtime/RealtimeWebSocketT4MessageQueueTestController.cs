...
{
    //* کنترلر تست تی‌فور است و صف شدن پیام مهم هنگام قطعی و فلش شدن بعد از ریکانکت را بررسی می‌کند.
    public class RealtimeWebSocketT4MessageQueueTestController : MonoBehaviour
    {
        [Header("Connection")]
        ...
        [Header("Room")]
        [SerializeField] private string roomIdPrefix = "unity_t4_room";
        [SerializeField] private string beforeDisconnectActionType = "unity_t4_before_disconnect";
        [SerializeField] private string queuedActionType = "unity_t4_queued_during_disconnect";
        [SerializeField] private string afterFlushActionType = "unity_t4_after_flush";
        ...
        //* اگر از اینسپکتور فعال باشد، تست تی‌فور را بعد از شروع صحنه اجرا می‌کند.
        private async void Start()
        {
            if (!runOnStart) return;
            await RunMessageQueueT4TestAsync();
        }
        ...
        //* از اینسپکتور یا دکمه یوآی برای اجرای تست صف پیام تی‌فور صدا زده می‌شود.
        public async void RunMessageQueueT4TestButton()
        {
            await RunMessageQueueT4TestAsync();
        }
        ...
        {
            await DisconnectAsync("Manual T4 disconnect");
        }
        ...
        public async Task<bool> RunMessageQueueT4TestAsync()
        {
            if (isRunning)
            {
                Log("T4 message queue test is already running.");
                return false;
            }

            isRunning = true;
            activeRoomId = BuildRunRoomId();
            Log("T4 message queue test started. room=" + activeRoomId);
            ...
            try
            {
                ...
                await DisconnectAsync("T4 intentional disconnect");
                ClearWaiters();
                await QueuePlayerActionDuringDisconnectAsync();

                if (realtimeClient.QueuedMessageCount <= 0) return Fail("Queue is empty after disconnected send.");
                Log("Queue count after disconnected send: " + realtimeClient.QueuedMessageCount);
                ...
                if (!left) return Fail("Leave room after T4 failed.");

                if (autoDisconnectAtEnd) await DisconnectAsync("T4 completed");

                Log("T4 message queue test completed successfully.");
                return true;
            }
            catch (OperationCanceledException)
            {
                return Fail("T4 message queue test canceled.");
            }
            catch (Exception ex)
            {
                return Fail("T4 message queue test exception: " + ex.Message);
            }
            finally
            {
                ClearWaiters();
        ...
        private async Task CleanupAsync()
        {
            ...
            if (realtimeClient != null) await realtimeClient.DisconnectAsync("T4 cleanup");
        ...
        {
            string prefix = string.IsNullOrWhiteSpace(roomIdPrefix) ? "unity_t4_room" : roomIdPrefix.Trim();
        ...
        {
            return "{\"source\":\"unity_t4\",\"phase\":\"" + EscapeJson(phase) + "\",\"roomId\":\"" + EscapeJson(activeRoomId) + "\",\"time\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}";
        }
...
//* این فایل تست صف پیام تی‌فور وب‌سوکت را برای یونیتی اجرا می‌کند.
//* تست ثابت می‌کند پیام مهم در زمان قطعی صف می‌شود و بعد از ریکانکت، آث و جوین دوباره با اَک ارسال می‌شود.
